...
from dice.sdk import Module, new_module
...
def aletheia_tag(mod: Module) -> None:
    ...
    q_python = query_records(
        "zmap",
        window__bt=[6370, 6379],
        tcpopt_wscale=64,
    )

    q_cloud = query_records(
        "zmap",
        window__bt=[502, 509],
        tcpopt_wscale__bt=[128, 256],
    )

    q_cloud_2 = query_records(
        "zmap",
    ...
    for q, d in [
        ...
        mod.itemize(
            q,
            lambda r: mod.store(
                mod.make_tag(r["saddr"], "aletheia", d, port=r["sport"])
            ),
            orient="rows",
        )
...
def make_aletheia_module() -> Module:
    return new_module("t", "aletheia", aletheia_tag, aletheia_init)
