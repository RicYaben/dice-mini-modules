...
def conpot_enip(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask = exploded.apply(
        lambda d: d.get("serial") == serial if isinstance(d, dict) else False
    )
...
def dicompot_dicom(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask_uinfo = (df["data_uid"].str.startswith("1.2.826.0.1.3680043.9.7133")) & (
        df["data_version"]
    ).str.startswith("GODICOM")

    # claims to be radiant, but UID not from that viewer
    mask_aet = (
        df["data_calling"].eq("radiant") | df["data_called"].eq("radiant")
    ) & ~df["data_uid"].str.startswith("1.2.826.0.1.3680043.8.1223", na=False)
...
dicompot_reg.add(
    new_module(
        "t",
        "dicom",
        make_hp_handler("DICOM", dicompot_dicom, "dicompot"),
        honeypot_init,
    )
)
...
conpot_reg.add(
    *[
        ...
        for p, filt in [
            ("iec104", conpot_iec104),
            ("ethernetip", conpot_enip),
            ("modbus", conpot_modbus),
        ]
    ]
)
...
honeygrove_reg.add(
    new_module(
        "t",
        "modbus",
        make_hp_handler("modbus", honeygrove_modbus, "honeygrove"),
        honeypot_init,
    )
)

honeypot_reg = registry("honeypot")
