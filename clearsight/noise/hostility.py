...
from dice.sdk import Flags, Module, flag, query
...
class TFlags(Flags):
    percentile: int = flag(
        95,
        "percentile to set threshold on the number of objects returned to consider a service hostile",
    )
...
tarpit = (
    registry("tarpit")
    .register(Module("t", "modbus", run_fn=modbus_run, flags=TFlags).add_tag(*ttag))
    .register(Module("t", "iec104", run_fn=iec104_run, flags=TFlags).add_tag(*ttag))
)

hostility = registry("hostility").add_group(tarpit)
